...
public class FileService {
	 ...
	 * - File 클래스 메서드
	 * 
	 * boolean  mkdir()          : 디렉토리 생성 => 1개만 생성
	 * boolean  mkdirs()         : 경로상의 모든 디렉토리 생성 => 2개 이상 생성 가능
	 ...
	 * 	   Mac	   : /Users/kkm/workspace/04_Java/12_io
	 * 
	 * * Java는 모든 운영체제에서 호환된다!
	 * => 코드에서 /, \ 를 신경쓰지 않아도 된다.
	 * => JVM 이 실행 시 실행되는 운영체제에 맞춰서 해석
	 * 
	 * - 최상위 경로(최상위 폴더, root)
	 * Window	 : C:, D:, E: (다수 존재 가능)
	 * Linux/Mac : /
	 */
...
//		File directory = new File("C:/workspace/04_Java/12_io/io_test/20250211");
		// => 백슬래시는 두개 써야되는데 슬래시는 하나만 써도 됨
		
		System.out.println("폴더 경로 : " + directory.getPath()); // 실제 폴더 없어도 경로 나옴
		System.out.println("폴더 유무 확인 : " + directory.exists()); // 폴더 없으면 false
		 
		if(!directory.exists()) {
			if(directory.mkdirs()) { // 생성 성공시 
			...
			for(File f : files) {
				System.out.println("파일들 : " + f);
				f.delete();
			}
			
			// 부모 폴더 삭제 시 자식 폴더/파일이 있으면 삭제 불가!!!!(내부에 파일이 있으면 삭제 불가)
	...
	public void test2() {
		...
		/* 절대 경로 : 절대적 기준(root 폴더) 부터 경로 표기 방식
		 *  
	...
	public void test3() {
		...
		for(File f : files) {
			...
			if(f.isFile()) {// 파일인 경우에만 크기측정
				long fileSize = fileName.length(); // 파일의 바이트 크기 반환
