 ...
 * (개발자들 사이의 규약, 약속 => 강제성은 없음)
 * => 왜? 언제든지 동일한 코드로 실행될 수 있게 하려고
 * 
 * - 인터페이스 작성법([] 는 생략이 가능한 부분)
 * 1) 상수형 필드만 작성 가능
...
public interface Calculator {
	...
	/* default 메서드 */
	/**
	 * 전달 받은 num이 최대/최소를 초과 하는지 확인
	 * - 최대값 초과 시 MAX_NUM을 반환
	 * - 최소값 미만 시 MIN_NUM을 반환
	 * - 범위 내 숫자는 그냥 반환
	 * @param num
	 */
	 ...
	 * @param num
	 */
	default double checkNum(double num) {
		if(num > MAX_NUM)  return MAX_NUM;
		if(num < MIN_NUM)  return MIN_NUM;
		return num;
	}
