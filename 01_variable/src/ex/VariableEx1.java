...
public class VariableEx1 {
	...
	*/
	public static void main(String[] args) {
		 ...
		 * 		 
		 * JS 에서 변수 자료형은 값이 대입되는 순간 결정
		 * 
		 * * Java는 JS 와 다르게 처음부터 변수의 자료형을 지정해야된다!!!
		 * 
		 * * byte = 8 bit
		 ...
		 * 
		 * - 논리형 : "boolean"(1byte) - true / false 저장 가능
		 * 
		 * - 정수형 : 양의 정수, 음의 정수, 0
		 * 			 1. "byte"(1byte)	 
		 * 			 2. "short"(2byte) 
		 ...
		 * 			 
		 * 			 4. "long"(8byte)
		 * 				=> 숫자 뒤에 L 또는 l을 붙임(L,l의 long 의 리터럴)	 
		 * 
		 * - 실수형 
		 * 			1. "float"(4byte)  
		 * 				=> 숫자 뒤에 F 또는 f를 붙임
		 ...
		 * 
		 * - 문자형
		 * 			1. "char"(2byte) : '문자 하나'를 지정하는 유니코드를 저장함
		 * 
		 */
		
		// 변수 선언
		int num1; // 메모리에 int형 정수를 저장할 공간 4byte를 할당하고 해당 공간(변수)의 이름을 num1 이라고 지정
		 ...
		 * (단, 대입 연산은 제외)
		 * */
//		The local variable num1 may not have been initialized 에러남
//		System.out.println(num1);
		...
		if(bool1 && !bool2) {
			System.out.println("이거 출력 될까? 된다리");
		}
		
		// 정수형
		// byte(1byte), short(2byte), int(4byte), long(byte)
		System.out.println("--- 정수형 ---");
		
		int intNum1 = 2147483647; // 인트 최대 값
		
//		int intNum2 = 2147483648; // The literal 2147483648 of type int is out of range
		// => int 범위 초과한 값을 대입 불가!
		// => 정수형 중에서 범위가 더 큰 long 을 사용!!!
		
		// 2147483648 <= 일반 정수 표기법 = int로 인식된다.
		// 2147483648l, 2147483648L 로 표기해야 Long 으로 인식된다.
		...
		// => 소수점 아래를 더 많이 표현 => 정확도가 높음 => 실수 기본형으로 채택
		...
		System.out.printf("%c, %d, %x \n",(int)ch1, (int)ch1, (int)ch1);
		// printf() : 출력될 문자열 형식을 지정한느 템플릿
		// %c : 문자형
		// %d : 10진수 정수형
		// %x : 16진수 정수형
		 ...
		 * - 값이 아닌 주소를 저장하는 "참조 자료형(참조형)" 변수
		 *  
		 *  - 리터럴 표기법 : "";
		 *  
		 *  - String은 몇 글자가 작성될지 모르기 때문에 기본적으로 크기가 정해져있는 
		 *  "기본 자료형"에 포함되지 않는다!!!
		 *  
		 *  - 대신 다른 곳에 문자열이 byte[] 모양으로 만들어지고 해당 배열의 주소를 참조하는 "참조형"이 된다!!
		 *  
		 *  */
		...
		System.out.println(System.identityHashCode(str1)); // hashCode : 주소 비슷한 것 => 주소가 다르면 같은 결과가 나올 수 없다!
