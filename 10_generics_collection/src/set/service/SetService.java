...
public class SetService {
	...
	public void test1() {
		...
		/* boolean remove(E e) :  */ //=> set은 순서가 없어서 index 번호로 삭제 못함
		// => 그래서 동등한 객체를 찾아서 제거함
		// => equals(), hashCode() 오버라이딩 필수
		System.out.println("쿠팡 제거됨? " + set.remove("쿠팡")); // true
		System.out.println("삼성 제거됨? " + set.remove("삼성")); // false
		System.out.println(set); // 지워짐 Ok
		
		/* boolean contains(E e) : 동등 객체 포함 여부 확인 */
		// => equals(), hashCode() 오버라이딩 필수
		System.out.println("네이버 있음? " + set.contains("네이버")); // true
		System.out.println("LG 있음? " + set.contains("LG")); // false
...
//		System.out.println(set == null); null 인건 아님. 내용물이 있는지 없는지 확인 하는 듯
		System.out.println("set : " + set);
		
		// null : 참조하는 객체가 없다. => 장바구니도 없음
		
		// 비어있다 : 참조하는 객체는 있는데 저장된 데이터가 없다. => 장바구니는 있는데 비어있음
		
		// "" (빈칸) : 빈칸을 저장 => 장바구니에 작성된 내용이 없는 문자열이 있음
		
		// 자바스크립트 undefined : 정의 되지 않음
...
//		System.out.println(list2.isEmpty()); // true
	...
	/**
	 * Hash 단어가 포함된 컬렉션은 무조건 equals(), hashCode()가 오버라이딩 되어있어야 한다!
	 * 
	 * @@@ Object가 제공하는 메서드 오버라이딩 목적 @@@
	 * 
	 * - equals() : 객체의 필드가 같은지 비교(동등 비교)
	 * 
	 * - hashCode() : 객체의 식별 번호 equals()가 true이면 hashCode()도 같아야 함
	 * 				  => 빠르게 찾기 위해 활용
	 */
	public void test2() {
...
//		Set<Person> personSet = new HashSet<Person>();
	 ...
	 * - <E> 반복자가 반환하는 요소의 유형
	 * 
	 * 1) boolean hasNext() : 다음 반복 접근할 요소가 있으면 true
	 * 
	 * 2) E next() : 다음 요소를 반환
	 * => 무조건 hasNext() 이후에 사용
	 */
	public void test3() {
		...
		// snacks(Set)에 저장된 값을 순차접근(반복접근)하는 객체 반환 받기
...
//		Set<String> set = new HashSet<String>();
	...
	// 로또 번호 6개가(중복X, 오름차순 정렬) 담긴 문자열 반환
...
//			int num = random.nextInt(45) + 1; //1 부터 45 사이 난수
//			
//			lotto[i] = num;			
//			for(int x = 0; x < i; x++) {
//				if(lotto[x] == num) {
...
//		System.out.println(Arrays.toString(lotto));
		...
		while(lotto.size() < 6) { // 저장된 값이 6개 미만이면 반복 = 저장된 값이 6개가 되면 반복 종료
